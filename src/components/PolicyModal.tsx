...
function PrivacyContent() {
    return (
        <div className="prose prose-sm max-w-none text-warm-300">
            ...
            <ul className="list-disc pl-5 space-y-1">
                <li>회원 가입 정보(이름, 번호 등): 해당 학년도 종료 시(다음 해 2월 말일)까지 또는 회원 탈퇴 시까지</li>
                <li>학습 결과물 및 연구 데이터: 수집일로부터 5년 (단, 개인을 식별할 수 없는 익명 처리된 상태로 보관하며 수업 연구 및 교육과정 개선 목적으로만 활용)</li>
            </ul>

            <h3 className="text-lg font-bold text-primary-700 mt-6 mb-3">제3조 (처리하는 개인정보 항목)</h3>
            <p>본 서비스는 학습 지원을 위해 필요한 최소한의 개인정보만을 수집합니다.</p>
            <ul className="list-disc pl-5 space-y-1">
                <li>수집 항목: 아이디, 비밀번호, 이름(또는 닉네임), 학년, 반, 번호, 학습 결과물, 평가 데이터, 연구 데이터, 그라운드(Grownd) 연동 식별자(지갑 주소 등)</li>
            ...
            <p>② 파기 방법은 다음과 같습니다.</p>
            <ul className="list-disc pl-5 space-y-1">
                <li>회원 정보: 전자적 파일 형태로 기록·저장된 개인정보(이름, 아이디 등)는 기록을 재생할 수 없도록 파기(DB 영구 삭제)합니다.</li>
                <li>연구용 데이터: 학습 결과물은 개인 식별 정보를 삭제하거나 복구 불가능한 형태로 가공(비식별화)하여 연구 자료로만 분리 보관합니다.</li>
            </ul>
            ...
            <p className="mt-2">본 서비스는 학습 보상 지급 및 외부 서비스 연동을 위해 다음과 같이 개인정보를 제3자에게 제공하고 있습니다.</p>
            <ul className="list-disc pl-5 space-y-1 mt-2">
                <li>제공받는 자: 그라운드(Grownd) 운영사</li>
                <li>제공 목적: 퀴즈 및 학습 달성에 따른 보상(포인트) 지급 및 서비스 연동</li>
                <li>제공하는 개인정보 항목: 그라운드(Grownd) 연동 식별자, 포인트 획득 내역</li>
                <li>보유 및 이용 기간: 그라운드(Grownd) 서비스 탈퇴 시 또는 앱뜰과의 연동 해제 시까지 (또는 그라운드 서비스의 개인정보 처리방침에 따름)</li>
            </ul>
